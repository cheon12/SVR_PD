...
class Control(PID):
    def __init__(self):
        ...
        self.data_z = {'goal_z':{'z' :0,'ddz':0 , 'F_E':0 },
                    'PID':{'z' : 50,'ddz':50,'F_E':30, 'control': 0}
                   }
    ...
    def V_cal(self):
        ...
        for key in self.data_z.keys():
            if key != 'goal_z':
                ...
                self.data_z[key]['F_E']+=self.data_z[key]['control']
        ...
        for key in self.data_x.keys():
            if key != 'goal_x':
                ...
                self.data_x[key]['ddx']=-(self.data_x['PID']['F_S'])/8.5
                self.data_x['PID']['F_S'] = max(min(self.data_x['PID']['F_S'], 50), -50)


        for key in self.data_theta.keys():
            if key != 'goal_theta':
                self.data_theta[key]['control']=V_control_theta[key]
                self.data_theta['PID']['pi']+=self.data_theta[key]['control']
                ...
                self.data_theta['PID']['pi'] = max(min(self.data_theta['PID']['pi'], 100), -100)
